...
const buildPublishedProjection = async (): Promise<string> => {
    const dir = await fs.mkdtemp(Path.join(os.tmpdir(), "node-esm-imports-"));
...
/**
 * Generated modules must not use extensionless relative import specifiers.
 * Bun's resolver accepts them, Node's ESM resolver does not, so a projection
 * published as `"type": "module"` fails on Node with ERR_MODULE_NOT_FOUND (and
 * ERR_UNSUPPORTED_DIR_IMPORT for a directory barrel). The check runs the real
 * `node` binary — under bun the defect is invisible.
 *
 ...
 */
describe("Generated modules under the Node ESM resolver", async () => {
