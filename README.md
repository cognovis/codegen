...
Repeated requests for the same resolved `name@version` are deduplicated. Calling `.fromPackage(name)` without a version requests `latest`, which the package manager resolves once before output; generated provenance and references use the concrete resolved version. The current canonical manager resolves one version per package name, so explicitly requesting different versions of the same root package fails with a diagnostic naming both identities. Manifest dependency declarations remain inputs to the package manager's resolution policy and may be normalized with JSON `forceDependencies`. If a resolved input nevertheless contains the same package canonical from different concrete versions, TypeSchema fails rather than silently collapsing them. Supported distinct package identities retain separate collision-safe directories and imports.
