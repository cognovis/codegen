...
- Zero or more explicit Bead IDs plus the repository that tracks them.
...
- Exact Bead-owned containers stopped before any Git integration or finalization side effect.
- Proof that the caller's committed head is contained, explicit Bead finalization,
  an Open Brain memory reference, and cleanup results.
- One recorded delivery identity: session ID, the exact Bead IDs, the repository,
...
Human-authority publication returns review_pending with Beads open; it is not
...
2. Change to a stable checkout outside any worktree that Cleanup must remove. Run
   ...
   the repository, branch, exact `--bead` and `--worktree` values, `--summary`, and
   each material `--learning`. Pass EVERY repository the session touched: one
   `ccore session-close run` per repository, never only the feature worktree's
   repository.
   Beads live in each repository's own embedded Dolt engine: run `bd show <id> --json`
   in the explicit `--bead-repo <repo>` and pass `--bead-repo <repo>`
   once it resolves the exact Bead. Block when it does not.
3. The first capability validates the explicit Beads and stops their exact opted-in containers.
...
4. Capture the returned Session Close ID. On a typed refusal, repair only the named condition and
   ...
   `finalize` verifies every close in the Bead repository and runs the journaled
   sync there; do not run a separate local Bead pull alongside the workflow.
...
- Session Close never force-pushes and never infers Bead IDs or cleanup targets
