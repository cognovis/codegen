...
NOTE: Most of these tests are currently skipped because the generated
``_systolic_slice_match`` / ``_diastolic_slice_match`` discriminators in the
Python profile have a malformed ``coding`` shape (``list[list[dict]]`` instead
of ``list[dict]``). This causes ``create()`` to raise a Pydantic validation
error when seeding component stubs. See the ``observation_uscore_blood_pressure_profile.py``
module — the ``_*_slice_match`` class attributes need to be regenerated with
the correct shape before these tests can be unskipped.
"""
...
# Reason text reused by the many tests blocked on the slice_match shape bug.
_CREATE_BROKEN_REASON = (
    "TODO: UscoreBloodPressureProfile.create() raises a Pydantic ValidationError because "
    "_systolic_slice_match / _diastolic_slice_match have coding shaped as list[list[dict]] "
    "instead of list[dict]. Regenerate the profile with the correct slice match shape."
)

# Reason text for tests blocked on the choice-type unwrap gap.
_CHOICE_UNWRAP_REASON = (
    "TODO: Python set_systolic / set_diastolic do not wrap the input under a value[x] choice "
    "variant (e.g. value_quantity), and get_systolic / get_diastolic do not unwrap it. The "
    "TS profile exposes a flat Quantity API; Python helpers wrap_slice_choice / unwrap_slice_choice "
    "exist but are not wired into the generated component slice methods."
)
...
@pytest.mark.skip(reason=_CHOICE_UNWRAP_REASON)
...
@pytest.mark.skip(reason=_CREATE_BROKEN_REASON)
...
def test_set_systolic_replaces_an_existing_systolic_component():
    profile = _make_bp()
    profile.set_systolic({"value_quantity": {"value": 130, "unit": "mmHg"}})
    obs = profile.to_resource()
    assert len(obs.component) == 2
...
@pytest.mark.skip(
    reason="TODO: Python set_systolic requires a value argument — TS supports a no-arg call "
    "that inserts the discriminator-only stub component."
)
