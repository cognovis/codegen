...
| Virtual R4 `Base` (nested elements) | A logical model whose element type is the virtual R4 `Base` must not fail `resolveFsGenealogy`; nested fields stay representable and unrelated unknown types still fail. Real trigger: `de.cognovis.fhir.dental@0.53.0` (`Failed to resolve FHIR Schema: ...Base`). The cognovis-fhir consumer build needs it; contributed upstream as [#236](https://github.com/atomic-ehr/codegen/pull/236), so drop this delta when it merges. |
| Deterministic TypeScript profile names | Profiles of one package that share a generated module or class name form one collision group. The highest canonical URL in code-unit order keeps the shared names; every other member is generated under a canonical-derived name, with a numeric suffix when that name is also taken, and one log report per group names all canonicals. Upstream writes by `StructureDefinition.name` with last-writer-wins, so the surviving definition depended on the package set. Real trigger: adding `hl7.fhir.eu.eps`/`hl7.fhir.uv.ips` to the cognovis-fhir release changed 7 r4-core classes. Not yet contributed upstream. |
| Nested base types owned by the specialization | A constraint profile's nested type reuses the specialization's nested URL and package, so the TypeSchema index keeps one owner per nested key: the defining specialization, then another specialization, then a profile, with equal ranks decided by owner package and canonical in code-unit order. Upstream registers every nested type with last-writer-wins, so slice choice variants and choice wrapping came from whichever profile was written last. Real trigger: `dental-chart-state` restricting `Observation.component.value[x]` truncated the `SliceFlat` types of 31 Observation profiles in the cognovis-fhir release, and adding `hl7.fhir.eu.eps`/`hl7.fhir.uv.ips` changed them. Not yet contributed upstream. |
...
| Virtual R4 `Base` (nested) | [#236](https://github.com/atomic-ehr/codegen/pull/236): keep nested elements typed as the virtual R4 `Base`. Built on `upstream/main` `2cd28e09`; upstream #209/#215 cover the schema base only. Retires the retained delta when merged. |
