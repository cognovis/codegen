...
Run it from a clean `main` checkout — not from a task worktree, which the script refuses.
...
1. **Dry run.** Fetches `upstream` and reports the checkout, `HEAD`, `upstream/main`, how many commits `main` is ahead (the fork commits listed above) and behind, and whether a merge is needed. It changes nothing. `--no-fetch` reports against the refs already on disk.
