...
var isFhirBaseCanonical = (canonical) => canonical.split("|")[0] === FHIR_BASE_CANONICAL;
...
var registerFromManager = async (manager, { logger, focusedPackages }) => {
  ...
  const resolveFsGenealogy = (pkg, canonicalUrl) => {
    ...
    while (fs6?.base) {
      ...
      if (fs6.kind === "logical" && fs6.derivation === "specialization" && isFhirBaseCanonical(baseUrl)) break;
...
var mkField = (register, fhirSchema, path, element, logger, rawElement) => {
  ...
  if (!valueConstraint && elemForCodingCheck.elements?.coding?.slicing?.slices) {
    ...
    const allRequired = allSliceValues.length > 0 && allSliceValues.every(
      (s) => s.min !== void 0 && s.min >= 1 && s.match && typeof s.match === "object" && typeof s.match.system === "string" && typeof s.match.code === "string"
    );
    if (allRequired) {
...
function transformFhirSchema(register, fhirSchema, logger) {
  ...
  if (fhirSchema.base) {
    ...
    const baseFs = register.resolveFs(fhirSchema.package_meta, baseUrl);
    const isVirtualLogicalBase = fhirSchema.kind === "logical" && fhirSchema.derivation === "specialization" && isFhirBaseCanonical(baseUrl);
